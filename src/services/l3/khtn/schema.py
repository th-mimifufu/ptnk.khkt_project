...
class AdmissionInputType1c(BaseModel):
    """
    Input cho Đối tượng 1c: Vì hiện tại chỉ test với hs của PTNK nên mặc định user là học trường chuyên.
    - Hạnh kiểm/điểm rèn luyện tốt trong 03 năm lớp 10, 11 và 12 (TẠI VÌ L2 CÓ LỌC HẠNH KIỂM VÀ HỌC LỰC RỒI)
    - Kết quả học tập được đánh giá mức tốt (đạt học lực giỏi) 03 năm học lớp 10, 11 và 12. (BỎ QUA TRƯỜNG HỢP CÓ 1 NĂM HỌC SINH KHÁ VÌ KHÔNG ĐỦ DATA)
    """
...
class AdmissionInputType1d(BaseModel):
    """
    Input cho Đối tượng 1d: 
    - Có chứng chỉ tiếng Anh
    - Kết quả học tập và hạnh kiểm được đánh giá mức tốt (đạt học lực giỏi) 03 năm học lớp 10, 11 và 12. (L2)
    """
    tnthpt_scores: TNTHPTScores
    high_school_grades: HighSchoolTranscript
